...
public class JavaGenerator extends AbstractGenerator {
    ...
    private void buildClassifierStructure(Classifier classifier,
    	...
    	System.out.println("Building class: #" + classifier.getName());
        // write class comment
    ...
    private void buildOperation(Operation operation, StringBuilder builder) {
    	System.out.println("Building operation:" + operation.getName());
        // write the visibility modifier if available
        ...
        if (operation.getOwner() instanceof Interface || operation.isAbstract()) {
            ...
            builder.append(SPACE).append(LEFT_BRACE);
            if (!this.buildCodeStub(this.getGeneralDocumentation(operation), builder)) {
     ...
     *            {@link StringBuilder} to aggregate the generated code
     */
    private boolean buildCodeStub(String doc, StringBuilder builder) {
    	if (doc == null) {
    		return false;
    	}
        Scanner scanner = new Scanner(doc);
        boolean hasImplementationNote = false;
        while (scanner.hasNext()) {
        	String line = scanner.nextLine();
        	if (!hasImplementationNote) {
        		hasImplementationNote = isImplementLine(line);
        	} else {
        		builder.append("        ").append(line).append(LINE_SEPARATOR);
        	}
        }
        return hasImplementationNote;
    }
    
    private boolean isImplementLine(String line) {
    	String testString = line.replaceAll("[^a-zA-Z0-9]", "").toLowerCase();
		if (testString.equals("implnote") || testString.equals("implementationnote")
				|| testString.equals("implementationnotes") || testString.equals("implnotes")
				|| testString.equals("implementation") || testString.equals("implementations")) {
			return true;
		}
		return false;
    }
